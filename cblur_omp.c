...
struct RGB sumRGB(float** kernel, struct Image copy, int i, int j, int edges, int thread_count)
{
    struct RGB pixel;
    int x,y;
    float sumRed = 0.0;
    ...
    pixel.red = sumRed / counter;
    pixel.green = sumGreen / counter;
    pixel.blue = sumBlue / counter;
...
void RGBImageToBlur(struct Image pic, int n, int thread_count)
{
    omp_set_num_threads(thread_count);
    int chunk = (int)floor(pic.width / thread_count);
    // printf("%d\n", chunk);
    int i, j, x, y;
    ...
    for (i = 0; i < pic.height; i++)
    {
        ...
        for (j = 0; j < pic.width; j++)
        {
            ...
            pic.rgb[i][j] = sumRGB(kernel, copy, i, j, edges, thread_count);
        }
    }
}
